...
class Generator:
    ...
    def generate(self, task_name, dataset_path):
        if self.args.model == "Phind/Phind-CodeLlama-34B-v2" and task_name == "output_prediction":
            task = tasks.get_task(task_name, dataset_path, cot = self.args.cot, phind_output = True)
        else:
            task = tasks.get_task(task_name, dataset_path, cot = self.args.cot, phind_output = False)
        ...
        sampling_params = SamplingParams(
            ...
            use_beam_search=self.args.use_beam_search,
        )

        ds_loader = DataLoader(ds_tokenized, batch_size=1)
        generations, generations_raw, logprobs_raw = complete_code(
        ...
        return generations, generations_raw, logprobs_raw, references