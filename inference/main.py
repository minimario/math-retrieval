...
def main():
    ...
    generations, generations_raw, logprobs_raw, references = generator.generate(args.task_name, args.dataset_path)
    ...
    # save logprobs
    path = args.save_generations_path
    path = path.split(".json")[0] + "_logprobs" + ".json"
    with open(path, "w") as fp:
        json.dump(logprobs_raw, fp)
        print(f"logprobs were saved at {path}")
