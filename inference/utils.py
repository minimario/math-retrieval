...
def complete_code(
    ...
    log_probs_raw = defaultdict(list)
    ...
    for step, batch in tqdm(enumerate(dataloader), total=total):
        ...
        generated_logprobs = [(o.text, o.cumulative_logprob) for o in outputs[0].outputs]

        combined_texts = [
            batch["prompt"][0] + generated_text for generated_text in generated_texts
        ]

        for task_idx, text, logprobs in zip(generated_tasks, combined_texts, generated_logprobs):
            ...
            log_probs_raw[task_idx].append(logprobs)

    return code_gens, code_gens_raw, log_probs_raw